...
def spectrum_plotter_with_L20andL24_with_QCstates():
    ...
    for i in range(0,len(irrep_list),1):
        ...
        QC_drive = threebody_path + "/test_files/QC_states_twoparams_fitting_L_20_24/"
        QC_file1 = QC_drive + "QC_states_jackknifed_L_20_nP_" + moms + ".dat"
        QC_file2 = QC_drive + "QC_states_jackknifed_L_24_nP_" + moms + ".dat"

        non_int_list = np.genfromtxt(non_int_file,unpack=True)
        L1, E1, Eerr1, EerrA = np.genfromtxt(spectrum_file1,unpack=True)
        L2, E2, Eerr2, EerrA2 = np.genfromtxt(spectrum_file2,unpack=True) 

        QCL1, QCspec1, QCspecErr1 = np.genfromtxt(QC_file1,unpack=True)
        QCL2, QCspec2, QCspecErr2 = np.genfromtxt(QC_file2,unpack=True) 
...
#spectrum_plotter_with_L20andL24()
