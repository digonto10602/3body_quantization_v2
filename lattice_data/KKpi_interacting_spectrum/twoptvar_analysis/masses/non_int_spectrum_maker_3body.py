...
def full_nonint_spectrum_maker_final(atmk, atmpi, xi, max_nsq, irrep, Lmin, Lmax, Lpoints, energy_file):
    ...
    #we get the full channel.energies files and the unique irrep list 
    full_energy_list, irrep_list = irrep_list_maker(energy_file)
    #We wont use the energy file anymore, we set the L value by hand
    Lbyas = 24 #int(full_energy_list[0][0])
    print("L = ",Lbyas)
    #irrep = irrep_list[5]
    print("irrep = ",irrep)
    ...
    for i in range(0,config_num,1):
        for j in range(0,config_num,1):
            onebyxi = 1.0/xi 
            pi = np.pi
            twopibyLbyas = 2.0*pi/Lbyas 
            frame_nsquare = frame_nx*frame_nx + frame_ny*frame_ny + frame_nz*frame_nz 
    ...
    non_int_L_points_file = "non_int_L" + str(int(Lbyas)) + "_points_" + str(irrep) + ".dat"
    
    f = open(non_int_L_points_file,"w")

    for i in range(0,len(unique_energy_df),1):
        f.write(    str(Lbyas) + '\t' 
    ...
    for i in range(0,len(sorted_energy_df[0]),1):
    ...
    new_selected_config_num = len(selected_particle1_config)
    out_file_list = np.zeros((new_selected_config_num+1,Lpoints))#np.zeros((config_num*config_num+1,Lpoints)) 
    L = np.linspace(Lmin,Lmax,Lpoints)

    count = 0
    for i in range(0,len(selected_particle1_config),1):
    ...
    for i in range(0,len(out_file_list[0]),1):
        for j in range(0,len(out_file_list),1):
            f.write(str(out_file_list[j][i]) + '\t')
            #print(out_file_list[i][j], sep=",")
        f.write('\n')
