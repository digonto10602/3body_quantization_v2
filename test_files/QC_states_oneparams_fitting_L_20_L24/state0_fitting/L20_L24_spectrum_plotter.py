...
def spectrum_plotter_with_L20andL24_with_QCstates():
    plt.rcParams.update({'font.size': 22,'legend.fontsize': 12})
    plt.rc('font',**{'family':'serif','serif':['Computer Modern Roman']})
    #plt.rc('font',**{'family':'serif','serif':['Palatino']})
    plt.rc('text', usetex=True)
    ...
    for i in range(0,len(irrep_list),1):
        ...
        QC_drive = threebody_path + "/test_files/QC_states_oneparams_fitting_L_20_L24/state0_fitting/"
        QC_file1 = QC_drive + "QC_states_jackknifed_L_20_nP_" + moms + "_energycutoff_0.39_one_param.dat"
        QC_file2 = QC_drive + "QC_states_jackknifed_L_24_nP_" + moms + "_energycutoff_0.39_one_param.dat"
    ...
    output_file = "KKpi_spectrum_plot_with_L20_and_L24_with_QC_states.pdf"
    output_file1 = "KKpi_spectrum_plot_with_L20_and_L24_with_QC_states.png"
...
def diff_plotter():
    ...
    for i in range(0,len(irrep_list),1):
        QC_drive = threebody_path + "/test_files/QC_states_oneparams_fitting_L_20_L24/state0_fitting/"
        QC_file1 = QC_drive + "QC_states_jackknifed_L_20_nP_" + mom_list[i] + "_energycutoff_0.39_one_param.dat"
        QC_file2 = QC_drive + "QC_states_jackknifed_L_24_nP_" + mom_list[i] + "_energycutoff_0.39_one_param.dat"

        QCL1, QCspec1, QCspecErr1, diff1, diff1err = np.genfromtxt(QC_file1,unpack=True)
        QCL2, QCspec2, QCspecErr2, diff2, diff2err = np.genfromtxt(QC_file2,unpack=True) 

        ax[0].errorbar(QCspec1[0], diff1[0] , xerr=None, yerr=diff1err[0],
                    marker=marker_list[i],markerfacecolor="None", markersize=20, color="red",
                    linestyle='none',markeredgewidth=1, capsize=20,zorder=4)
        
        ax[0].plot(QCspec1[0], diff1[0] , 
        ...
        ax[1].errorbar(QCspec2[0], diff2[0] , xerr=None, yerr=diff2err[0],
                    marker=marker_list[i],markerfacecolor="None", markersize=20, color="red",
                    linestyle='none',markeredgewidth=1, capsize=20,zorder=4)
        ax[1].plot(QCspec2[0], diff2[0] , 
                    marker=marker_list[i],markerfacecolor="white", markersize=20, color="red",
                    linestyle='none', markeredgewidth=1, zorder=5,label=mom_list[i])
        
        ax[0].errorbar(QCspec1[1:], diff1[1:] , xerr=None, yerr=diff1err[1:],
                    marker=marker_list[i],markerfacecolor="None", markersize=20, color="grey",
                    linestyle='none',markeredgewidth=1, capsize=20,zorder=4)
        
        ax[0].plot(QCspec1[1:], diff1[1:] , 
                    marker=marker_list[i],markerfacecolor="white", markersize=20, color="grey",
                    linestyle='none', markeredgewidth=1, zorder=5)

        ax[1].errorbar(QCspec2[1:], diff2[1:] , xerr=None, yerr=diff2err[1:],
                    marker=marker_list[i],markerfacecolor="None", markersize=20, color="grey",
                    linestyle='none',markeredgewidth=1, capsize=20,zorder=4)
        ax[1].plot(QCspec2[1:], diff2[1:] , 
                    marker=marker_list[i],markerfacecolor="white", markersize=20, color="grey",
                    linestyle='none', markeredgewidth=1, zorder=5)
    ...
    fig.tight_layout()
    outfile = "diff_plot.pdf"
    outfile1 = "diff_plot.png"
    plt.savefig(outfile)
...
spectrum_plotter_with_L20andL24_with_QCstates()
